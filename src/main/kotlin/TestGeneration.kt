...
internal class TestGenerator(
    ...
    // Since this class is internal anyway, generators is not private for testing purposes.
    /** The generators that this TestGenerator needs. Keys are the class generated by the value. */
    val generators: Map<Class<*>, Gen<*>> = setUpGenerators()
    ...
    private fun setUpGenerators(): Map<Class<*>, Gen<*>> {
        ...
        val generatorMapBuilder = object {
            private var knownGenerators: MutableMap<Class<*>, Lazy<Gen<*>>> = mutableMapOf()
            private val requiredGenerators: Set<Class<*>> = run {
                val goalTypes = types.toSet().toMutableSet()

                if (!Modifier.isStatic(reference.modifiers) && nextReceiver == null) {
                    goalTypes += referenceClass
                }

                return@run if (referenceClass in goalTypes) {
                    goalTypes + submissionClass
                } else {
                    goalTypes
                }.also { it.forEach(::request) }
            }
            ...
            fun build(): Map<Class<*>, Gen<*>> {
                requiredGenerators.forEach {
                    knownGenerators[it]?.value ?: throw lazyGenError(it)
                }

                return mapOf(*requiredGenerators.map { it to (knownGenerators[it]?.value ?: throw lazyGenError(it)) }.toTypedArray())
            }
        }
        ...
        return mapOf(
                *types.filter { it.isArray }.map { type ->
                    Pair(type, LazyGen { DefaultArrayGen(generators[type.componentType] ?: throw AnswerableMisuseException("An array with component type `${type.componentType}' was requested, but no generator for that type was found.")) })
                }.toTypedArray(),
                *gens.toTypedArray()
        )*/
    }
